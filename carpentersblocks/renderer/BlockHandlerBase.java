...
public class BlockHandlerBase implements ISimpleBlockRenderingHandler
{
	...
	/*
	 * Overrides for various rendering components.
	 */
	protected boolean	isSideCover = false,
						renderAlphaOverride = CarpentersBlocks.enableZFightingFix,
	...
	protected boolean	isFaceSloped = false;
	...
	public boolean renderWorldBlock(IBlockAccess blockAccess, int x, int y, int z, Block srcBlock, int modelID, RenderBlocks renderBlocks)
    {
    	 ...
    	 * Render fancy fluids
    	 */
    	if (renderPass >= 0 && CarpentersBlocks.enableFancyFluids && Minecraft.isFancyGraphicsEnabled() && BlockProperties.hasCover(TE, 6))
	...
	{
		return	srcBlock == CarpentersBlocks.blockCarpentersDoor ||
				srcBlock == CarpentersBlocks.blockCarpentersHatch;
	}
	...
	protected boolean shouldRenderOverlay(TECarpentersBlock TE, int renderPass)
	{
		if (!suppressOverlay)
		{
			Block coverBlock = BlockProperties.getCoverBlock(TE, sideRendering);		

			if (BlockProperties.hasOverlay(TE, sideRendering) || coverBlock == Block.grass) {
	...
	protected boolean shouldRenderPattern(TECarpentersBlock TE, int renderPass)
	{
		...
		{
			if (renderPass == 1)
				if (BlockProperties.hasPattern(TE, sideRendering))
					return true;
		}
	...
	private void renderFancyFluids(TECarpentersBlock TE, RenderBlocks renderBlocks, int x, int y, int z, int renderPass)
	{
    	 ...
    	 */
    	if (CarpentersBlocks.enableFancyFluids)
    	{
    		Block	block_XN = !renderBlocks.blockAccess.isAirBlock(x - 1, y, z) ? Block.blocksList[renderBlocks.blockAccess.getBlockId(x - 1, y, z)] : null,
    				block_XP = !renderBlocks.blockAccess.isAirBlock(x + 1, y, z) ? Block.blocksList[renderBlocks.blockAccess.getBlockId(x + 1, y, z)] : null,
    				block_ZN = !renderBlocks.blockAccess.isAirBlock(x, y, z - 1) ? Block.blocksList[renderBlocks.blockAccess.getBlockId(x, y, z - 1)] : null,
    				block_ZP = !renderBlocks.blockAccess.isAirBlock(x, y, z + 1) ? Block.blocksList[renderBlocks.blockAccess.getBlockId(x, y, z + 1)] : null,
    				block_XZNP = !renderBlocks.blockAccess.isAirBlock(x - 1, y, z + 1) ? Block.blocksList[renderBlocks.blockAccess.getBlockId(x - 1, y, z + 1)] : null,
			...
			boolean isSolid_XN = renderBlocks.blockAccess.isBlockSolidOnSide(x, y, z, ForgeDirection.WEST, true),
					...
					isSolid_ZN = renderBlocks.blockAccess.isBlockSolidOnSide(x, y, z, ForgeDirection.NORTH, true),
					isSolid_ZP = renderBlocks.blockAccess.isBlockSolidOnSide(x, y, z, ForgeDirection.SOUTH, true);
    	...
    	for (int side = 0; side < 6; ++side)
    	{
    		...
    		{	    		
	    		...
	    		if (shouldRenderBlock(TE, renderBlocks, coverBlock, srcBlock, renderPass) || shouldRenderPattern(TE, renderPass))
	    		{
					...
					setMetadataOverride(BlockProperties.getCoverMetadata(TE, side));
    ...
    protected Icon setupIcon(TECarpentersBlock TE, RenderBlocks renderBlocks, Block coverBlock, Block srcBlock, int side, int x, int y, int z)
    {
		...
		else if (srcBlock == CarpentersBlocks.blockCarpentersDaylightSensor)
			icon = srcBlock.getBlockTexture(renderBlocks.blockAccess, x, y, z, side);
		else
			icon = coverBlock.getBlockTexture(renderBlocks.blockAccess, x, y, z, isFaceSloped ? 2 : side);
    	
    	// Default icon for Carpenter's Lever is the lever itself, so we'll replace it here.
    	if (coverBlock == CarpentersBlocks.blockCarpentersLever)
    		icon = IconHandler.icon_generic;
    	    	
    	if (isFaceSloped)
    	{
    		if (Slope.isObliqueCorner(data) && !BlockProperties.hasCover(TE, 6)) {
    			if (Slope.isObliqueInteriorCorner(data))
    				icon = Slope.isPositive(data) ? IconHandler.icon_slope_oblique_pt_low : IconHandler.icon_slope_oblique_pt_high;
    			else
    				icon = Slope.isPositive(data) ? IconHandler.icon_slope_oblique_pt_high : IconHandler.icon_slope_oblique_pt_low;
    		}

    		// For grass material, use top texture on positive slope side
    		if (Slope.isPositive(data) && coverBlock.blockMaterial == Material.grass)
    ...
    protected void renderMultiTexturedSide(TECarpentersBlock TE, RenderBlocks renderBlocks, Block coverBlock, Block srcBlock, int side, int x, int y, int z, float lightness, Icon icon)
    {
		 ...
		 */
		if (srcBlock == CarpentersBlocks.blockCarpentersDaylightSensor && side == 1)
			return;
    ...
    {    	
    	...
    	if (hasDyeColorOverride) {
        	...
        	dyeRGB = DyeColorHandler.getDyeColorRGB(suppressDyeColor ? DyeColorHandler.NO_COLOR : BlockProperties.getDyeColor(TE, sideRendering));
    	}
    	...
    	if (hasLightnessOffset)
    	{
    		if (renderBlocks.enableAO) {    			
    			...
    			rgb_no_color[0] += lightnessOffset;
    			rgb_no_color[1] += lightnessOffset;
    			rgb_no_color[2] += lightnessOffset;
    		}
    	}
    	...
    	if (renderBlocks.enableAO) {
    		...
    		if (useColorComponent(TE, coverBlock, srcBlock, side, icon))
    			tessellator.setColorOpaque_F(rgb_no_color[0] * baseRGB[0] * dyeRGB[0], rgb_no_color[1] * baseRGB[1] * dyeRGB[1], rgb_no_color[2] * baseRGB[2] * dyeRGB[2]);
    		else
    			tessellator.setColorOpaque_F(rgb_no_color[0] * dyeRGB[0], rgb_no_color[1] * dyeRGB[1], rgb_no_color[2] * dyeRGB[2]);
    		
    	}
    }
    
    /**
     * Returns whether block side color component should be ignored.
     */
    protected boolean useColorComponent(TECarpentersBlock TE, Block coverBlock, Block srcBlock, int side, Icon icon)
    {
    	if (coverBlock == Block.grass || BlockProperties.getOverlay(TE, side) == OverlayHandler.OVERLAY_GRASS)
    	{
			if (side == 1 || icon == BlockGrass.getIconSideOverlay())
				return srcBlock == CarpentersBlocks.blockCarpentersDaylightSensor ? side != 1 : true;
			else
				return false;
    	}
    ...
    {
    	int data = BlockProperties.getData(TE);
    	int overlay = BlockProperties.getOverlay(TE, sideRendering);
		boolean isSide = (!isFaceSloped && side > 1) || (isFaceSloped && Slope.isNegative(data));
		
		suppressDyeColor = true;
		
		// Do not render overlay if side is bottom and overlay drapes
		if (side == 0 && !isFaceSloped && (overlay == OverlayHandler.OVERLAY_GRASS || overlay == OverlayHandler.OVERLAY_SNOW || overlay == OverlayHandler.OVERLAY_HAY || overlay == OverlayHandler.OVERLAY_MYCELIUM))
			return;
    ...
    {
    	int pattern = BlockProperties.getPattern(TE, sideRendering);
    ...
    protected float[] getRGB(IBlockAccess blockAccess, Block block, int x, int y, int z)
    {
    	...
    	int color = CarpentersBlocks.enableOptifineIntegration ? OptifineHandler.getColorMultiplier(block, blockAccess, x, y, z) : block.colorMultiplier(blockAccess, x, y, z);
    ...
    protected boolean renderStandardBlock(TECarpentersBlock TE, RenderBlocks renderBlocks, Block coverBlock, Block srcBlock, int x, int y, int z)
    {
    	...
    	if (srcBlock == CarpentersBlocks.blockCarpentersSlope && !isSideCover) {
    		
            if (Minecraft.isAmbientOcclusionEnabled() && Block.lightValue[coverBlock.blockID] == 0)
            	renderStandardSlopeWithAmbientOcclusion(TE, renderBlocks, coverBlock, srcBlock, x, y, z, rgb[0], rgb[1], rgb[2], renderBlocks.partialRenderBounds);
            else
            ...
            if (Minecraft.isAmbientOcclusionEnabled() && !disableAO && Block.lightValue[coverBlock.blockID] == 0)
            	renderStandardBlockWithAmbientOcclusion(TE, renderBlocks, coverBlock, srcBlock, x, y, z, rgb[0], rgb[1], rgb[2], renderBlocks.partialRenderBounds);
            else
     ...
     */
    protected boolean renderStandardSlopeWithAmbientOcclusion(TECarpentersBlock TE, RenderBlocks renderBlocks, Block coverBlock, Block srcBlock, int x, int y, int z, float f, float g, float h, boolean partialRenderBounds)
    {
		return false;
	}
     ...
     */
    protected boolean renderStandardBlockWithAmbientOcclusion(TECarpentersBlock TE, RenderBlocks renderBlocks, Block coverBlock, Block srcBlock, int x, int y, int z, float red, float green, float blue, boolean maxSmoothLighting)
    {	
        ...
        {
        	computeLightnessYNeg(renderBlocks, coverBlock, x, y, z, maxSmoothLighting);
        ...
        {
        	computeLightnessYPos(renderBlocks, coverBlock, x, y, z, maxSmoothLighting);
        ...
        {
        	computeLightnessZNeg(renderBlocks, coverBlock, x, y, z, maxSmoothLighting);
        ...
        {
        	computeLightnessZPos(renderBlocks, coverBlock, x, y, z, maxSmoothLighting);
        ...
        {
        	computeLightnessXNeg(renderBlocks, coverBlock, x, y, z, maxSmoothLighting);
        ...
        {
        	computeLightnessXPos(renderBlocks, coverBlock, x, y, z, maxSmoothLighting);
     ...
     */
    protected void computeLightnessYNeg(RenderBlocks renderBlocks, Block coverBlock, int x, int y, int z, boolean maxSmoothLighting)
    {
     ...
     */
    protected void computeLightnessYPos(RenderBlocks renderBlocks, Block coverBlock, int x, int y, int z, boolean maxSmoothLighting)
    {
    	int mixedBrightness = coverBlock.getMixedBrightnessForBlock(renderBlocks.blockAccess, x, y, z);
     ...
     */
    protected void computeLightnessZNeg(RenderBlocks renderBlocks, Block coverBlock, int x, int y, int z, boolean maxSmoothLighting)
    {
        int mixedBrightness = coverBlock.getMixedBrightnessForBlock(renderBlocks.blockAccess, x, y, z);
     ...
     */
    protected void computeLightnessZPos(RenderBlocks renderBlocks, Block coverBlock, int x, int y, int z, boolean maxSmoothLighting)
    {
        int mixedBrightness = coverBlock.getMixedBrightnessForBlock(renderBlocks.blockAccess, x, y, z);
     ...
     */
    protected void computeLightnessXNeg(RenderBlocks renderBlocks, Block coverBlock, int x, int y, int z, boolean maxSmoothLighting)
    {
        int mixedBrightness = coverBlock.getMixedBrightnessForBlock(renderBlocks.blockAccess, x, y, z);
     ...
     */
    protected void computeLightnessXPos(RenderBlocks renderBlocks, Block coverBlock, int x, int y, int z, boolean maxSmoothLighting)
    {
        int mixedBrightness = coverBlock.getMixedBrightnessForBlock(renderBlocks.blockAccess, x, y, z);
    ...
    protected boolean renderStandardBlockWithColorMultiplier(TECarpentersBlock TE, RenderBlocks renderBlocks, Block coverBlock, Block srcBlock, int x, int y, int z, float red, float green, float blue)
    {
        ...
        if (renderBlocks.renderAllFaces || srcBlock.shouldSideBeRendered(renderBlocks.blockAccess, x, y - 1, z, 0) || renderBlocks.renderMinY > 0.0D)
        {
    		...
    		rgb_no_color = rgb_YN;
        	prepareRender(TE, renderBlocks, coverBlock, srcBlock, 0, x, y, z, 0.0F);
        ...
        if (renderBlocks.renderAllFaces || srcBlock.shouldSideBeRendered(renderBlocks.blockAccess, x, y + 1, z, 1) || renderBlocks.renderMaxY < 1.0D)
        {
    		...
    		rgb_no_color = rgb_YP;
        	prepareRender(TE, renderBlocks, coverBlock, srcBlock, 1, x, y, z, 0.0F);
        ...
        if (renderBlocks.renderAllFaces || srcBlock.shouldSideBeRendered(renderBlocks.blockAccess, x, y, z - 1, 2) || renderBlocks.renderMinZ > 0.0D)
        {
    		...
    		rgb_no_color = rgb_Z;
        	prepareRender(TE, renderBlocks, coverBlock, srcBlock, 2, x, y, z, 0.0F);
        ...
        if (renderBlocks.renderAllFaces || srcBlock.shouldSideBeRendered(renderBlocks.blockAccess, x, y, z + 1, 3) || renderBlocks.renderMaxZ < 1.0D)
        {
    		...
    		rgb_no_color = rgb_Z;
        	prepareRender(TE, renderBlocks, coverBlock, srcBlock, 3, x, y, z, 0.0F);
        ...
        if (renderBlocks.renderAllFaces || srcBlock.shouldSideBeRendered(renderBlocks.blockAccess, x - 1, y, z, 4) || renderBlocks.renderMinX > 0.0D)
        {
    		...
    		rgb_no_color = rgb_X;
        	prepareRender(TE, renderBlocks, coverBlock, srcBlock, 4, x, y, z, 0.0F);
        ...
        if (renderBlocks.renderAllFaces || srcBlock.shouldSideBeRendered(renderBlocks.blockAccess, x + 1, y, z, 5) || renderBlocks.renderMaxX < 1.0D)
        {
    		...
    		rgb_no_color = rgb_X;
        	prepareRender(TE, renderBlocks, coverBlock, srcBlock, 5, x, y, z, 0.0F);
