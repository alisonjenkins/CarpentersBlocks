...
public class BlockBase extends BlockContainer
{
	...
	public void onBlockClicked(World world, int x, int y, int z, EntityPlayer entityPlayer)
	{
		...
		if (itemStack != null)
		{
			...
			if (item.equals(CarpentersBlocks.itemCarpentersHammer))
			{
				...
				if (entityPlayer.isSneaking())
				{
					...
					if (!world.isRemote)
					{
						if (BlockProperties.hasOverlay(TE, effectiveSide)) {
							...
							BlockProperties.setDyeColor(TE, effectiveSide, 0);
							dataAltered = true;
			...
			} else if (item.equals(CarpentersBlocks.itemCarpentersChisel)) {
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer entityPlayer, int side, float hitX, float hitY, float hitZ)
	{
		...
		if (itemStack != null)
		{
			...
			if (itemStack.getItem() == CarpentersBlocks.itemCarpentersHammer) {

				actionPerformed = onHammerRightClick(TE, entityPlayer, side);

			} else if (CarpentersBlocks.enableChisel && itemStack.getItem() == CarpentersBlocks.itemCarpentersChisel) {
			...
			} else if (CarpentersBlocks.enableCovers && BlockProperties.isCover(itemStack)) {

				if (!BlockProperties.hasCover(TE, 6)) {

					BlockProperties.setCover(TE, 6, itemStack);
					actionPerformed = true;
					decrementInventory = true;

				} else if (CarpentersBlocks.enableSideCovers) {

					if (!BlockProperties.hasCover(TE, side) && this.canCoverSide(TE, world, x, y, z, side)) {

						BlockProperties.setCover(TE, side, itemStack);
						actionPerformed = true;
						decrementInventory = true;
			...
			} else if (CarpentersBlocks.enableOverlays && BlockProperties.isOverlay(itemStack)) {
				...
				if (!BlockProperties.hasOverlay(TE, effectiveSide) && (effectiveSide < 6 && BlockProperties.hasCover(TE, effectiveSide) || effectiveSide == 6)) {
					...
					BlockProperties.setOverlay(TE, effectiveSide, itemStack);
					actionPerformed = true;
					decrementInventory = true;

				}

			} else if (CarpentersBlocks.enableDyeColors && itemStack.getItem() == Item.dyePowder && itemStack.getItemDamage() != 15) {
				...
				if (!BlockProperties.hasDyeColor(TE, effectiveSide)) {
					...
					BlockProperties.setDyeColor(TE, effectiveSide, (15 - itemStack.getItemDamage()));
					actionPerformed = true;
					decrementInventory = true;
		...
		if (decrementInventory && !world.isRemote)
	...
	public int isProvidingWeakPower(IBlockAccess world, int x, int y, int z, int side)
    {
		if (!willCoverRecurse(world, x, y, z))
		{
			...
			int power = BlockProperties.getCoverBlock(world, 6, x, y, z).isProvidingWeakPower(world, x, y, z, 6);
			
			int sidePower = 0;
			for (int temp_side = 0; temp_side < 6; ++temp_side)
				if (BlockProperties.hasCover(TE, temp_side))
					sidePower = BlockProperties.getCoverBlock(TE, temp_side).isProvidingWeakPower(world, x, y, z, temp_side);
	...
	public int isProvidingStrongPower(IBlockAccess world, int x, int y, int z, int side)
    {
		if (!willCoverRecurse(world, x, y, z))
		{
			...
			int power = BlockProperties.getCoverBlock(world, 6, x, y, z).isProvidingStrongPower(world, x, y, z, 6);
			
			int sidePower = 0;
			for (int temp_side = 0; temp_side < 6; ++temp_side)
				if (BlockProperties.hasCover(TE, temp_side))
					sidePower = BlockProperties.getCoverBlock(TE, temp_side).isProvidingStrongPower(world, x, y, z, temp_side);
	...
	private boolean suppressDestroyBlock(EntityPlayer entityPlayer, ItemStack heldItem)
	{
		return	entityPlayer.capabilities.isCreativeMode &&
				heldItem != null &&
				(
					heldItem.getItem() == CarpentersBlocks.itemCarpentersHammer ||
					heldItem.getItem() == CarpentersBlocks.itemCarpentersChisel
				);
	}
	...
	public boolean addBlockDestroyEffects(World world, int x, int y, int z, int metadata, EffectRenderer effectRenderer)
	{
		...
		{
	        /*
	         * Reach distance (from PlayerControllerMP):
	         * 
	         * Creative - 5.0F
	         * Survival - 4.5F
			 */
			EntityPlayer entityPlayer = world.getClosestPlayer(x, y, z, 5.0F);
    ...
    {
    	if (CarpentersBlocks.enablePlantSupport)
    	{
